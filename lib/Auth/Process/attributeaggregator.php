            ...
            foreach ($config["attributes"] as $attribute) {
                ...
                if (array_key_exists("values", $attribute)) {
                    if (!is_array($attribute["values"])) {
                        throw new Exception(
                            "attributeaggregator: Invalid format of attributes array in the configuration"
                        );
                    }
                }
            ...
            try {
                ...
                if (empty($this->aaEndpoint)) {
                    throw new \RuntimeException(
                        $this->entityId . ' does not have a usable AttributeService endpoint in metadata' .
    ...
    public function process(&$state): void
    {
        ...
        try {
            ...
            if (empty($state['Attributes'][$this->attributeId]) ||
                ...
                throw new \RuntimeException(
                    "Can't attempt attribute query, attribute " .
            ...
            $query->setID($randomUtil->generateID());
            ...
            if (!$response->isSuccess()) {
                throw new \RuntimeException(
                    'Got a SAML error on attribute query (' .
